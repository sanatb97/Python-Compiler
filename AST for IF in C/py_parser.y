...
%token <nam> IF ELSE ELIF
%token <nam> LE GE LT GT EQ NE NUM AND OR TR FL STRLT ID
%type <gen> B C
%type <nam> relop Q T F E assign Arg P I R TER N G S EL
%start P
%left '+' '-' '*' '/'
%right '=' '^'
...
P : I ID {insert_type_table();} '('{add('t');} R ')'{add('t');} '{'{add('t');} S { printf("Label next:\n");}  U '}'{$$.nd = mknode(NULL,$12.nd,"start");printf("___________________________________________________________________________________________n");printf("\n\t\t\tSyntax Tree in Inorder traversal\n___________________________________________________________________________________________\n");
printtree($$.nd);
printf("\n\n");
add('t');};
I : I I | INCLUDE {add('H');} ;
R : R ','{add('t');} R | N TER | N TER;
...
S :	IF{add('k');} '(' C ')'{printf("\nLabel\t%s:\n",$4.tr);} S;
	...
	strcpy($$.name,"IF");}
	|assign {$$.nd=$1.nd;}
	|ID TER {$$.nd=mknode(NULL,NULL,"definition"); int i=sym_search($2.name);if(i!=-1)
